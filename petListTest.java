...
public class petListTest extends Common {
	...
	@Test(enabled = true, groups = { SMOKE }, description = "description")
	public void Test_Positive_MultipleStatus() {
		Map<String, String> queryParams = new TreeMap<>();
		queryParams.put("status", "available,sold");

		Response response = getUrl(findByStatus, queryParams);
		Assert.assertEquals(response.getStatusCode(), 200, "Invalid response code");
		Assert.assertTrue(response.getTime() < MAX_RESPONSE_TIME, "Response time is greater than expected");
	}
}
